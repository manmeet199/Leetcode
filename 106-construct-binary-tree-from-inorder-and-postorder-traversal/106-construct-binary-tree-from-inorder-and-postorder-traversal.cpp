...
public:
    ...
    if(is>ie || ps>pe)
    {
        ...
        int inorderRoot=hm[root->val];
        int numsleft=inorderRoot-is;
        root->left=buildTreePostIn(inorder,is,inorderRoot-1,postorder,ps,ps+numsleft-1,hm);
       root->right=buildTreePostIn(inorder,inorderRoot+1,ie,postorder,ps+numsleft,pe-1,hm);
        return root;
    }
};